# Owaiken - AI Agent Builder

<img src="public/Owaiken.png" alt="Owaiken Logo" />
...
> **🔄 IMPORTANT UPDATE (April 6th)**: Owaiken now includes a library of prebuilt tools, examples, and MCP server integrations. Owaiken can now incorporate these resources when building new agents, significantly enhancing capabilities and reducing hallucinations. Note that the examples/tool library for Owaiken is just starting out. Please feel free to contribute examples, MCP servers, and prebuilt tools!

Owaiken is an advanced **"Agenteer"**, an AI agent designed to autonomously build, refine, and optimize other AI agents. 

It serves both as a practical tool for developers and as an educational framework demonstrating the evolution of agentic systems.
Owaiken is developed in iterations, starting with just a simple Pydantic AI agent that can build other Pydantic AI agents,
all the way to a full agentic workflow using LangGraph that can build other AI agents with any framework.
Through its iterative development, Owaiken showcases the power of planning, feedback loops, and domain-specific knowledge in creating robust AI agents.

## Important Links

- The current version of Owaiken is V6 as mentioned above - see [V6 Documentation](iterations/v6-tool-library-integration/README.md) for details.

- Join our [Owaiken community forum](https://discord.gg/owaiken) to ask questions and share your experiences!

- [GitHub Kanban board](https://github.com/users/coleam00/projects/1) for feature implementation and bug squashing.

## Vision

Owaiken demonstrates three key principles in modern AI development:
...
Since V6 is the current version of Owaiken, all the code for V6 is in both the main directory and `owaiken/iterations/v6-tool-library-integration` directory.

Note that the examples/tool library for Owaiken is just starting out. Please feel free to contribute examples, MCP servers, and prebuilt tools!
...
- **Chat**: Interact with Owaiken to create AI agents
...
If you encounter any errors when using Owaiken, please first check the logs in the "Agent Service" tab.
...
#### Option 1: Docker
To get the latest updates for Owaiken when using Docker:

```bash
# Pull the latest changes from the repository (from within the owaiken directory)
git pull
...
- Detect and remove any existing Owaiken containers (whether running or stopped)
- Rebuild the containers with the latest code
- Start fresh containers with the updated version

#### Option 2: Local Python Installation
To get the latest updates for Owaiken when using local Python installation:
...
This ensures you're always running the most recent version of Owaiken with all the latest features and bug fixes.
...
- Docker support
- Comprehensive Streamlit interface for managing all aspects of Owaiken
...
### Future Iterations
- V7: LangGraph Documentation - Allow Owaiken to build Pydantic AI AND LangGraph agents
...
- V13: MCP Agent Marketplace - Integrating Owaiken agents as MCP servers and publishing to marketplaces
...
## Owaiken Agents Architecture

The below diagram from the LangGraph studio is a visual representation of the Owaiken agent graph.

<img src="public/OwaikenGraph.png" alt="Owaiken Graph" />
...
4. Control is passed back to you to either give feedback or ask Owaiken to 'refine' the agent autonomously
...
8. Once the agent is complete, Owaiken spits out the full code again with instructions for running it
...
- `workbench/`: Created at runtime, files specific to your environment
  ...
  - `logs.txt`: Low level logs for all Owaiken processes go here
  - `scope.md`: The detailed scope document created by the reasoner model at the start of each Owaiken execution
