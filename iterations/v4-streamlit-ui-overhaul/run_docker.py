...
def main():
    ...
    try:
        result = subprocess.run(
            ["docker", "ps", "-q", "--filter", "name=owaiken-container"],
            check=True,
            capture_output=True,
            text=True
        )
        if result.stdout.strip():
            print("\n=== Stopping existing Owaiken container ===")
            run_command(["docker", "stop", "owaiken-container"])
            run_command(["docker", "rm", "owaiken-container"])
