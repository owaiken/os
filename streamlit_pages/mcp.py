...
def generate_mcp_config(ide_type, python_path, server_script_path):
    ...
    if ide_type == "Windsurf":
        ...
        return f"{python_path} {server_script_path}", f"docker run -i --rm --add-host=host.docker.internal:host-gateway -e GRAPH_SERVICE_URL=http://host.docker.internal:8100 owaiken-mcp:latest"
...
def mcp_tab():
    ...
    if st.session_state.selected_ide:
        ...
        if selected_ide == "Windsurf":
            ...
            1. Deploy and run Owaiken in Docker
            2. In the Owaiken UI, start the MCP service.
            3. Open a terminal and navigate to your work folder.
            4. Execute the command: 
            
            \tFor Docker: `claude mcp add Owaiken docker run -i --rm -e GRAPH_SERVICE_URL=http://host.docker.internal:8100 owaiken-mcp:latest `   
            \tFor Python: `claude mcp add Owaiken {python_path} {server_script_path}`
            
            5. Start Claude Code with the command `claude`. When Claude Code starts, at the bottom of the welcome section will be a listing of connected MCP Services, Owaiken should be listed with a status of _connected_.
            6. You can now use the Owaiken MCP service in your Claude Code projects
