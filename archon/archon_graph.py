...
from owaiken.pydantic_ai_coder import pydantic_ai_coder, PydanticAIDeps
from owaiken.advisor_agent import advisor_agent, AdvisorDeps
from owaiken.refiner_agents.prompt_refiner_agent import prompt_refiner_agent
from owaiken.refiner_agents.tools_refiner_agent import tools_refiner_agent, ToolsRefinerDeps
from owaiken.refiner_agents.agent_refiner_agent import agent_refiner_agent, AgentRefinerDeps
from owaiken.agent_tools import list_documentation_pages_tool
...
async def advisor_with_examples(state: AgentState):
    # Get the directory one level up from the current file (owaiken_graph.py)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(current_dir)
    
    # The agent-resources folder is adjacent to the parent folder of owaiken_graph.py
